/************************************************************************************//**
* \file         Source/RISCV_CH32V30x/flash.c
* \brief        Bootloader flash driver source file.
* \ingroup      Target_RISCV_CH32V30x_flash
* \internal
*----------------------------------------------------------------------------------------
*                          C O P Y R I G H T
*----------------------------------------------------------------------------------------
...
/************************************************************************************//**
* \defgroup   Target_RISCV_CH32V30x_flash Flash driver of a port
...
#define BOOT_FLASH_VECTOR_TABLE_CS_OFFSET    (0x1a4)
#endif
...
static const tFlashSector flashLayout[] =
{
   ...
   * for the bootloader can be made a lot smaller here. Sectors om this target can be 
   * erased in 256/4k/32K/64K chunks, 64K chunks are used wherever it is possible.
   */
  /* { 0x08000000, 0x04000,  0},           flash sector  0 - reserved for bootloader   */
  /* { 0x08004000, 0x04000,  1},           flash sector  1 - reserved for bootloader   */
  { 0x08008000, 0x08000,  0},           /* flash sector  2 - 32kb                      */
#if (BOOT_NVM_SIZE_KB > 64)
  { 0x08010000, 0x10000,  1},            /* flash sector  3 - 64kb                      */
#endif
#if (BOOT_NVM_SIZE_KB > 128)
  { 0x08020000, 0x10000,  2},            /* flash sector  4 - 64kb                      */
  { 0x08030000, 0x10000,  3},            /* flash sector  5 - 64kb                      */
#endif
#if (BOOT_NVM_SIZE_KB > 256)
  { 0x08040000, 0x10000,  4},            /* flash sector  6 - 64kb                      */
  { 0x08050000, 0x10000,  5},            /* flash sector  7 - 64kb                      */
  { 0x08060000, 0x10000,  6},            /* flash sector  8 - 64kb                      */
  { 0x08070000, 0x08000,  7},            /* flash sector  9 - 32kb                      */
#endif
#if (BOOT_NVM_SIZE_KB > 480)
#error "BOOT_NVM_SIZE_KB > 480 is currently not supported."
#endif
};
#else
...
blt_bool FlashWriteChecksum(void)
{
  ...
  /* for the RISCV_CH32V307 target we defined the checksum as the Two's complement value of the
   ...
   *    0x08000010 Reserved
   *    0x08000014 Ecall_M_Mode_Handler
   *    0x08000018 Reserved
   *
...
static blt_bool FlashWriteBlock(tFlashBlockInfo *block)
{
  ...
  if (result == BLT_TRUE)
  {
    /* This microcontroller has a fast flash writing function where 256 bytes can be written at once */
    for (word_cnt=0; word_cnt<(FLASH_WRITE_BLOCK_SIZE/256u); word_cnt++)
    {
      prog_addr = block->base_addr + (word_cnt * 256u);
      /* keep the watchdog happy */
      CopService();
      if (FLASH_ROM_WRITE(prog_addr,(uint32_t *)&block->data[word_cnt * 256u], 256u) != FLASH_COMPLETE)
      {
        result = BLT_FALSE;
        break;
      }
  ...
  if (result == BLT_TRUE)
  {
    ...
    for (sectorIdx=first_sector; sectorIdx<= last_sector; sectorIdx++)
    {
      ...
      if (FlashEmptyCheckSector(sectorIdx) == BLT_FALSE)
      {
        ...
        if (FLASH_ROM_ERASE(flashLayout[sectorIdx].sector_start, flashLayout[sectorIdx].sector_size) != FLASH_COMPLETE)
        {
