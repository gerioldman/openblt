...
void MbRtuInit(void)
{
  ...
  /* Initialize free running 100kHz timer */
  RCC_APB1PeriphClockCmd( RCC_APB1Periph_TIM6, ENABLE );

  RCC_ClocksTypeDef RCC_Clocks;
  RCC_GetClocksFreq(&RCC_Clocks);

  uint32_t TIM_Prescaler = (RCC_Clocks.PCLK1_Frequency / 100000u) -1u;
  ...
  /* Initialize pins and UART peripheral */
  ...
  USART_InitStructure.USART_BaudRate = BOOT_COM_MBRTU_BAUDRATE;
#if(BOOT_COM_MBRTU_STOPBITS == 1)
  USART_InitStructure.USART_StopBits = USART_StopBits_1;
#elif(BOOT_COM_MBRTU_STOPBITS == 2)
...
static blt_bool MbRtuReceiveByte(blt_int8u *data)
{
  ...
  if (USART_GetFlagStatus(USART_CHANNEL, USART_FLAG_RXNE) == SET)
  {
    /* update the result */
    result = BLT_TRUE;
    /* Check for a frame error. This is typically done by checking the frame
     ...
     * a frame error check.
     */
    if (USART_GetFlagStatus(USART_CHANNEL, USART_FLAG_FE) == SET)
    {
      /* clear the error flag. */
      USART_ClearFlag(USART_CHANNEL, USART_FLAG_FE);
     ...
     */
    if (USART_GetFlagStatus(USART_CHANNEL, USART_FLAG_PE) == SET)
    {
      /* clear the error flag. */
      USART_ClearFlag(USART_CHANNEL, USART_FLAG_PE);
      /* ignore the byte because of a detected frame error. */
      result = BLT_FALSE;
    }
    #endif
...
static void MbRtuTransmitByte(blt_int8u data, blt_bool end_of_packet)
{
  ...
  else
  {
     ...
     */
    while (USART_GetFlagStatus(USART_CHANNEL, USART_FLAG_TC) == RESET)
    {
      /* keep the watchdog happy. */
      CopService();
      /* break loop upon timeout. this would indicate a hardware failure. */
