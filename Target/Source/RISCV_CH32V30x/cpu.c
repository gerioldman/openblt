...
void CpuStartUserProgram(void)
{
  ...
  /* CH32V307 microcontrollers are based on RISC-V, therefore by standard don't have vector tables. 
   * However, WCH extended it, and it is done in the default startup files, so it can be omitted.
   * If the application startup files don't set it up by standard, then extend it here. 
   * Interrupts are enabled globally at startup by default. TODO: check if last sentence is actually true or not!
   */
  CpuIrqEnable();

  pProgResetHandler = (void*)(((blt_addr)CPU_USER_PROGRAM_STARTADDR_PTR) - (blt_addr)0x08000000u);
